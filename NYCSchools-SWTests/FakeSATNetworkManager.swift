...
final class FakeSATNetworkManager: NetworkProtocol {
    ...
    func fetchData<Response>(endpoint: NYCSchoolsEndpoint<Response>, useCache: Bool = false) async throws -> Response {
        ...
        do{
            let bundle = Bundle(for: FakeSATNetworkManager.self)
            guard let path = bundle.url(forResource: "MockSATTest", withExtension: "json") else{
